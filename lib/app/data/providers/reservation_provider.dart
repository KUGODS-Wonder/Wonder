...
class ReservationProvider extends GetConnect {
  ...
  Future<List<Reservation>> getReservations() async {
    ...
    try {
      ...
      if (response.success) {
        ...
        var reservationList = await _parseReservationData(reservationDataList);
  ...
  Future<int?> postReservation({required int voluntaryWorkId}) async {
    String? errorMessage;
  ...
  Future<List<Reservation>> _parseReservationData(List<ReservationData> voluntaryDataList) async {
    var reservationList = <Reservation>[];

    for (ReservationData data in voluntaryDataList) {
      reservationList.add(Reservation.fromData(data));
    }
