...
class WalkCardsWidget extends StatefulWidget {
  final List<Walk> walks;
  final Function(Walk walk) onWalkChange;

  const WalkCardsWidget({Key? key, required this.walks, required this.onWalkChange}) : super(key: key);

  @override
  WalkCardsWidgetState createState() => WalkCardsWidgetState();
}

class WalkCardsWidgetState extends State<WalkCardsWidget> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    _cardWidth = (size.width * .75).clamp(250.0, 350.0);
    _cardHeight = _cardWidth * 0.93;
  ...
  bool _handleScrollNotifications(Notification notification) {
    ...
    if (notification is ScrollUpdateNotification) {
      ...
      widget.onWalkChange(widget.walks.elementAt(_pageController.page!.round() % widget.walks.length));
    }
    //Scroll Start
  ...
  Widget _buildItemRenderer(int itemIndex) {
    return Container(
      ...
      child: Rotation3d(
        ...
        //   walk: widget.walks[itemIndex % widget.walks.length],
